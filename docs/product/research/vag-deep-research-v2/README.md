# Volkswagen Group deep research v2

**Pack id:** `vag-deep-research` · **Pack version:** 2 · **Migrated:** 2026-09-02
**Authoring contract:** [UDS brand research pack specification](../../../uds/brand-research-pack-specification.md) v1.0

This directory carries the same research as `vag-deep-research-v1`, rewritten
...
| No `claims[]` | 8 claims, one per execution-eligible source, each with an action, a promotion test and a boundary |
...
| Record | Count |
|---|---|
| Routes | 104 (104 authorized for automatic execution) |
| DID candidates | 217 (217 authorized) |
| Command evidence | 25 |
| Platforms | 14 |
| ECU families | 6 |
| Module inventories | 0 |
| Validation recipes | 9 |
| Conflicts / gaps | 5 / 16 |
| Sources | 13 (8 pinned to an immutable revision) |

## What could not be expressed

- 0 records had no single canonical identifier or no resolved
  response address. They are preserved as research leads in
  `conflicts-and-gaps.json` and generate no traffic.
- 3 source decoders describe a field layout the canonical
...
node --experimental-strip-types \
  packages/uds-map/scripts/migrate-legacy-research-pack.ts --brand vag \
  --input docs/product/research/vag-deep-research-v1 \
  --output docs/product/research/vag-deep-research-v2
```
