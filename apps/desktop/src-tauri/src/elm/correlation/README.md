...
2. **Reference fits.** Every distinct reference key is fitted, including the
   ...
   from -2,000 to +1,500 ms are evaluated, with deterministic ties favouring
   more samples and then the smaller lag. Pearson `r` describes association.
   Slope and intercept use a deterministic dominant-line estimator: widely
   separated point pairs vote for a slope bin, then medians reject stale
   sequential reads. Residual SD is computed against that robust line.
3. **Events.** Speed derivative below -1 m/s² creates the conservative
   braking reference. Nearby explicit brake-switch/pressure probe readings
   participate when supplied. Reports retain transition and A→B→A counts.
4. **Arrays.** Consecutive, equal-width sibling DIDs present in at least half
   the rounds form an array candidate. For four-member arrays, the pairing
   with the lowest within-pair squared difference is selected. With at least
   three left-turn samples above +45°, the faster pair becomes
   `outer_in_left_turn`.
...
- the explicit engine-off repeated-pedal capture shows monotonic depletion
  without recovery, discriminating servo vacuum (`0.82`).
