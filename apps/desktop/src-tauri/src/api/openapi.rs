...
pub const ROUTES: &[RouteDoc] = &[
    ...
    r("POST", "/rides", "Start a ride on the connected vehicle; 409 when disconnected or already recording", &[], None, false, false),
    r("POST", "/rides/{id}/stop", "Run the closing fault-code scan and stop the active ride", &[], None, false, false),
    r("GET", "/rides", "List recorded rides for a vehicle", &[("vehicle_id", "vehicle id")], None, false, false),
    r("GET", "/rides/{id}", "Get one recorded ride", &[], None, false, false),
