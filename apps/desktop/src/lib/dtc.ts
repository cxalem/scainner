...
//    ranked common causes, symptoms, severity) for the codes that actually
...
export const DTC_LIBRARY: Record<DtcCode, DtcInfo> = {
  ...
  P0117: { title: "Coolant temp sensor low", meaning: "Engine-coolant temperature signal is shorted low (reads extremely hot) — the ECU can't trust engine temperature.", causes: ["Failed ECT sensor", "Wiring short to ground", "Connector corrosion"], symptoms: ["Fans running constantly", "Hard starts", "Rich running"], severity: "medium" },
  P0118: { title: "Coolant temp sensor high", meaning: "Engine-coolant temperature signal reads open (extremely cold) — cold-start enrichment never ends.", causes: ["Failed ECT sensor", "Wiring open circuit", "Unplugged connector"], symptoms: ["High consumption", "Black smoke when warm", "Poor idle when warm"], severity: "medium" },
  P0128: { title: "Coolant below thermostat temperature", meaning: "The engine never reached proper operating temperature in the expected time — almost always a thermostat stuck open.", causes: ["Thermostat stuck open (typical)", "Failed ECT sensor", "Very low coolant level"], symptoms: ["Slow warm-up", "Weak cabin heat", "Higher consumption in winter"], severity: "low" },
  P0130: { title: "O2 sensor circuit (bank 1, sensor 1)", meaning: "The upstream oxygen sensor's signal is faulty — the ECU is losing its main feedback for fuel mixture.", causes: ["Aged/failed O2 sensor", "Wiring/connector damage (they run near hot parts)", "Exhaust leak near the sensor"], symptoms: ["Higher consumption", "Rough idle", "Emissions test failure"], severity: "medium" },
  P0135: { title: "O2 sensor heater circuit (B1S1)", meaning: "The upstream O2 sensor's built-in heater isn't working, so the sensor stays inactive until exhaust heat warms it — long open-loop running after start.", causes: ["Failed sensor heater element", "Blown heater fuse", "Wiring fault"], symptoms: ["Higher consumption on short trips", "Emissions test failure"], severity: "low" },
  P0171: { title: "System too lean (bank 1)", meaning: "The ECU keeps adding fuel beyond normal limits because the mixture reads lean — extra air is getting in, or fuel/measurement is short.", causes: ["Vacuum/intake leak (split hose, inlet gasket)", "Dirty MAF under-reading airflow", "Weak fuel pump or clogged filter", "Leaking injector seals"], symptoms: ["Rough idle, hesitation", "Positive fuel trims well above +10%", "Possible misfires under load"], severity: "medium" },
  P0172: { title: "System too rich (bank 1)", meaning: "The ECU keeps removing fuel beyond normal limits — the engine gets more fuel than the measured air justifies.", causes: ["Leaking/stuck injector", "Faulty fuel-pressure regulator", "MAF over-reading", "Stuck-closed purge valve logic issues"], symptoms: ["Fuel smell, black exhaust tint", "Negative fuel trims beyond −10%", "Fouled spark plugs over time"], severity: "medium" },
  ...
  P0301: { title: "Cylinder 1 misfire", meaning: "Cylinder 1 specifically is intermittently failing to fire. The fault follows one cylinder, which narrows the hunt to its own plug, coil, injector, or compression.", causes: ["Spark plug or ignition coil on cyl 1 (swap-test with another cylinder)", "Injector on cyl 1", "Low compression in cyl 1 (valve/rings)"], symptoms: ["Rhythmic shake at idle", "Hesitation", "Flashing MIL under load if severe"], severity: "high" },
  P0302: { title: "Cylinder 2 misfire", meaning: "Cylinder 2 specifically is intermittently failing to fire — same diagnosis logic as any single-cylinder misfire.", causes: ["Plug/coil on cyl 2 (swap test)", "Injector on cyl 2", "Compression on cyl 2"], symptoms: ["Shake at idle", "Power loss"], severity: "high" },
  P0303: { title: "Cylinder 3 misfire", meaning: "Cylinder 3 specifically is intermittently failing to fire — same diagnosis logic as any single-cylinder misfire.", causes: ["Plug/coil on cyl 3 (swap test)", "Injector on cyl 3", "Compression on cyl 3"], symptoms: ["Shake at idle", "Power loss"], severity: "high" },
  P0325: { title: "Knock sensor circuit", meaning: "The knock sensor's signal is faulty — the ECU protects the engine by retarding ignition timing conservatively.", causes: ["Failed knock sensor", "Wiring/connector damage", "Loose sensor mounting"], symptoms: ["Slightly reduced power", "Higher consumption", "Usually no drivability drama"], severity: "low" },
  P0335: { title: "Crankshaft position sensor", meaning: "The crank sensor signal is faulty — this is the ECU's primary timing reference; a dead one means no start.", causes: ["Failing sensor (often heat-related, intermittent)", "Damaged reluctor ring", "Wiring fault"], symptoms: ["Stalling", "No-start or long cranking", "Tachometer dropouts"], severity: "high" },
  P0340: { title: "Camshaft position sensor", meaning: "The cam sensor signal is faulty — the ECU may fall back to crank-only running: it starts, but less precisely.", causes: ["Failed cam sensor", "Wiring fault", "Timing chain/belt stretch changing correlation"], symptoms: ["Long cranking", "Slight power loss", "Rough running"], severity: "medium" },
  P0401: { title: "EGR flow insufficient", meaning: "The exhaust-gas-recirculation system isn't flowing as commanded — usually carbon clogging in the valve or passages.", causes: ["Carbon-clogged EGR valve/passages (typical, especially diesels)", "Stuck/failed EGR valve", "Faulty DPFE/flow sensor"], symptoms: ["Possible knock/pinging (petrol)", "Emission test failure", "Often no daily symptoms"], severity: "low" },
  P0420: { title: "Catalyst efficiency below threshold (bank 1)", meaning: "The rear O2 sensor sees almost the same signal as the front one — the catalytic converter is no longer storing/converting properly. Important: a tired catalyst is often the VICTIM of an upstream problem (misfires, rich running), not the root cause.", causes: ["Aged/degraded catalytic converter", "Misfires or rich mixture poisoning the cat (check other codes FIRST)", "Lazy front or rear O2 sensor giving a false reading", "Exhaust leak between the sensors"], symptoms: ["Usually none in daily driving", "Emissions inspection failure", "Sulphur smell in hard use if far gone"], severity: "medium" },
  P0430: { title: "Catalyst efficiency below threshold (bank 2)", meaning: "Same as P0420 but on the second cylinder bank of a V-engine.", causes: ["Aged catalytic converter (bank 2)", "Upstream mixture/misfire problems", "Lazy O2 sensors on bank 2"], symptoms: ["Usually none daily", "Emissions failure"], severity: "medium" },
  P0442: { title: "EVAP small leak", meaning: "The fuel-vapour system loses a small amount of pressure — a tiny leak somewhere between tank and purge valve.", causes: ["Loose or worn fuel filler cap (check first)", "Cracked EVAP hose", "Leaking purge/vent valve"], symptoms: ["None in driving", "Faint fuel smell sometimes"], severity: "low" },
  P0455: { title: "EVAP large leak", meaning: "The fuel-vapour system can't hold pressure at all — a big opening, most famously the filler cap left loose.", causes: ["Fuel cap loose/missing/bad seal (by far most common)", "Disconnected EVAP hose", "Failed vent valve"], symptoms: ["None in driving", "Fuel smell possible"], severity: "low" },
  P0500: { title: "Vehicle speed sensor", meaning: "The ECU isn't receiving a plausible road-speed signal.", causes: ["Failed VSS/wheel-speed source", "Wiring fault", "Instrument-cluster data issue"], symptoms: ["Speedometer dropouts", "Harsh/odd automatic shifting", "Cruise control disabled"], severity: "medium" },
  P0505: { title: "Idle control system", meaning: "The ECU can't regulate idle speed to target — the idle path (valve or electronic throttle) isn't responding correctly.", causes: ["Carbon-fouled throttle body/idle valve", "Vacuum leak", "Failed idle control actuator"], symptoms: ["Idle too high/low or hunting", "Stalling at stops"], severity: "medium" },
  P0562: { title: "System voltage low", meaning: "The ECU sees supply voltage below normal while running — the charging system isn't keeping up.", causes: ["Weak alternator or worn brushes", "Corroded battery terminals/grounds", "Aging battery dragging the bus down"], symptoms: ["Dim lights at idle", "Battery warning light", "Random electrical glitches"], severity: "high" },
  P0563: { title: "System voltage high", meaning: "Supply voltage above normal — the voltage regulator is overcharging, which cooks batteries and electronics.", causes: ["Failed voltage regulator/alternator", "Poor ground reference"], symptoms: ["Very bright lights", "Battery smell/heat", "Short bulb life"], severity: "high" },
  P0700: { title: "Transmission control system fault", meaning: "An umbrella code: the transmission control unit has stored its own fault and asked the engine ECU to turn the light on. The real detail lives in the TCU.", causes: ["Any transmission-side fault — read the TCU's own codes", "Wiring between TCU and ECU"], symptoms: ["Depends on the underlying TCU code", "Often limp mode or harsh shifts"], severity: "high" },
  U0100: { title: "Lost communication with ECM/PCM", meaning: "Another module can't reach the engine computer on the CAN bus — a network problem, not an engine problem.", causes: ["CAN wiring/connector fault", "Failing module pulling the bus down", "Low battery voltage during cranking (phantom entry)"], symptoms: ["Multiple warning lights at once", "Gauges dropping out", "Possible no-start"], severity: "high" },
